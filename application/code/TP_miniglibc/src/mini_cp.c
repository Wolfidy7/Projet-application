...
int mini_cp(char* source, char* dest) {


    MYFILE* src = mini_fopen(source, 'r');
    ...
    printf("test\n");
    MYFILE* dst = mini_fopen(dest, 'w');
    
    if( dst != NULL ){
        printf("mini_cp: cannot stat '%s': No such file or directory", dest);
        mini_exit();
    }
    printf("test\n");
